...
pd <- esp_df %>% mutate(iso = 'ESP',
                        cases_non_cum = confirmed_cases_non_cum) %>% bind_rows(ita %>% mutate(iso = 'ITA') %>% mutate(confirmed_cases = cases))
  
pd <- pd %>% group_by(iso, date) %>%
  summarise(uci = sum(uci, na.rm = TRUE),
            deaths = sum(deaths, na.rm = TRUE),
            cases = sum(confirmed_cases, na.rm = TRUE),
