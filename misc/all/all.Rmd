...
label_data <- pd %>% group_by(ccaa) %>% filter(
                                                          (
                                                            ...
                                                            (date == max(date) & country == 'España' & deaths_pm > 25) |
                                                              (date == max(date) & country == 'Italia' &
                                                                 ccaa != 'Liguria' & days_since_start_deaths_pm > 10)
                                                          ))
...
```{r}
african_isos <- world_pop %>% filter(region == 'Africa')
pd <- df_country %>%
          group_by(iso) %>%
         filter(max(confirmed_cases) > 0) %>%
         filter(iso %in% african_isos$iso) %>%
  mutate(country = ifelse(country == 'Central African Republic', 'C.A.R.', 
                          ifelse(grepl('Brazza', country), 'Congo, Rep.',
                                 ifelse(grepl('Kinsha', country), 'Congo, D.R.', 
                                        ifelse(grepl('Equatorial', country),
                                               'Equat. Guinea', country)))))
...
```{r}
latam_isos <- world_pop %>% filter(sub_region == 'Latin America and the Caribbean')
pd <- df_country %>%
          group_by(iso) %>%
         filter(max(confirmed_cases) > 0) %>%
         filter(iso %in% latam_isos$iso) 
cols <- colorRampPalette(brewer.pal(n = 8, 'Spectral'))(length(unique(pd$country)))
cols <- sample(cols, length(cols))
# Plot
ggplot(data = pd,
  ...
  labs(x = '',
       y = 'Confirmed cases',
       title = 'COVID-19 in Latin America and the Caribbean')
ggsave('~/Desktop/latam.pdf',
       height = 8,
       width = 12)
```
...
```{r}
southern_asia_isos <- world_pop %>% filter(sub_region == 'Southern Asia')
pd <- df_country %>%
          group_by(iso) %>%
         filter(max(confirmed_cases) > 0) %>%
         filter(iso %in% southern_asia_isos$iso) 
cols <- colorRampPalette(brewer.pal(n = 8, 'Spectral'))(length(unique(pd$country)))
cols <- sample(cols, length(cols))
# Plot
ggplot(data = pd,
  ...
  labs(x = '',
       y = 'Confirmed cases',
       title = 'COVID-19 in Southern Asia')
ggsave('~/Desktop/southern_asia.pdf',
       height = 8,
       width = 12)
```
...
```{r}
isos <- world_pop %>% filter(sub_region == 'Eastern Europe')
pd <- df_country %>%
          group_by(iso) %>%
         filter(max(confirmed_cases) > 0) %>%
         filter(iso %in% isos$iso) 
cols <- colorRampPalette(brewer.pal(n = 8, 'Spectral'))(length(unique(pd$country)))
cols <- sample(cols, length(cols))
# Plot
ggplot(data = pd,
  ...
  labs(x = '',
       y = 'Confirmed cases',
       title = 'COVID-19 in Eastern Europe')
ggsave('~/Desktop/eastern_europe.pdf',
       height = 8,
       width = 12)
```
