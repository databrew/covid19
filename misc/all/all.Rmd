...
## Africa and Latin America for ISGlobal

```{r}
dir.create('~/Desktop/isglobal/')
...
pd
ggsave('~/Desktop/isglobal/a_africa_two_weeks_ago.png',
       height = 5.4,
       width = 9.6)
...
pd <- plot_day_zero(countries = c(african_countries),
                    ...
                    max_date = Sys.Date() - 6)
pd
ggsave('~/Desktop/isglobal/b_africa_1_week_ago.png',
       height = 5.4,
       width = 9.6)
...
pd <- plot_day_zero(countries = c(african_countries),
                    ...
                    max_date = Sys.Date())
pd
ggsave('~/Desktop/isglobal/c_africa_now.png',
       height = 5.4,
       width = 9.6)
...
pd <- plot_day_zero(countries = c(latam_countries),
                    ...
                    max_date = Sys.Date() - 13)
pd
ggsave('~/Desktop/isglobal/d_latam_two_weeks_ago.png',
       height = 5.4,
       width = 9.6)
...
pd <- plot_day_zero(countries = c(latam_countries),
                    ...
                    max_date = Sys.Date() - 6)
pd
ggsave('~/Desktop/isglobal/e_latam_1_week_ago.png',
       height = 5.4,
       width = 9.6)
...
pd <- plot_day_zero(countries = c(latam_countries),
                    ...
                    max_date = Sys.Date())
pd
ggsave('~/Desktop/isglobal/f_latam_now.png',
       height = 5.4,
       width = 9.6)
...
g
ggsave('~/Desktop/isglobal/g_europe_only.png',
       height = 5.4,
       width = 9.6)
...
g + 
    geom_line(data = pd %>% filter(region == 'Africa'),
            ...
            alpha = 0.8) +
    scale_color_manual(name = '', values = cols) 
ggsave('~/Desktop/isglobal/h1_europe_and_africa.png',
       height = 5.4,
       width = 9.6)
...
g + 
    geom_line(data = pd %>% filter(sub_region == 'Latin America and the Caribbean'),
            ...
            alpha = 0.8) +
    scale_color_manual(name = '', values = cols) 
ggsave('~/Desktop/isglobal/h2_europe_and_latam.png',
       height = 5.4,
       width = 9.6)
...
g + 
    geom_line(data = pd %>% filter(sub_region != 'Europe'),
            ...
            alpha = 0.8) +
    scale_color_manual(name = '', values = cols) 
ggsave('~/Desktop/isglobal/h3_europe_latam_and_africa.png',
       height = 5.4,
       width = 9.6)
...
cols <- rev(colorRampPalette(c('darkred', 'red', 'darkorange', 'orange', 'yellow', 'white'))(10))
g <- ggplot(data = map,
         aes(x = long,
             y = lat,
             group = group)) +
  ...
  labs(title = 'Cumulative COVID-19 deaths per million population (Spain and bordering regions)',
  ...
  geom_text(data = centroids,
            ...
            size = 3)
g
ggsave('~/Desktop/map_with_borders.pdf')
```
...
```{r}
dir.create('~/Desktop/animation_map/')
pd <- por_df %>% mutate(country = 'Portugal') %>%
  bind_rows(esp_df %>% mutate(country = 'Spain')) %>%
  bind_rows(fra_df %>% mutate(country = 'France')) %>%
  bind_rows(ita %>% mutate(country = 'Italy')) %>%
...
for(i in 1:length(unique_dates)){
  ...
  # cols <- c('#009392','#39b185','#9ccb86','#e9e29c','#eeb479','#e88471','#cf597e')
  # cols <- c('#008080','#70a494','#b4c8a8','#f6edbd','#edbb8a','#de8a5a','#ca562c')
  cols <- rev(colorRampPalette(c('darkred', 'red', 'darkorange', 'orange', 'yellow', 'white'))(15))
  ...
  ggsave(paste0('~/Desktop/animation_map/', this_date, '.png'),
         plot = g,
         height = 6, width = 10)
}
```
...
ggsave('~/Desktop/spain_non_cum.png')
...
ggsave('~/Desktop/spain_ayer_adj.png')
